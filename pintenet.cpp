...
struct ThreadData {
    ...
    ADDRINT mem_r2_addr;
    ADDRINT mem_r2_size;
    ADDRINT mem_mov_size;
    ADDRINT mem_mov_addr;
...
VOID GetArgMemmove(THREADID tid,void* dest,  size_t size, VOID *v){
    auto &context = *reinterpret_cast<ToolContext *>(v);
    ThreadData *data = context.GetThreadLocalData(tid);
    data->mem_mov_addr = (ADDRINT)dest;
    data->mem_mov_size = size;

    OutFile1 << "MEMMOVE DEST:" << setbase(16) << dest << "  SIZE:" << size << endl;
}
VOID GetArgMemcpy(THREADID tid,void* dest,  size_t size, VOID *v){
    auto &context = *reinterpret_cast<ToolContext *>(v);
    ThreadData *data = context.GetThreadLocalData(tid);
    data->mem_mov_addr = (ADDRINT)dest;
    data->mem_mov_size = size;

    OutFile1 << "MEMCPY DEST:" << setbase(16) << dest << "  SIZE:" << size << endl;
}
...
static VOID OnImageLoad(IMG img, VOID *v) {
    ...
    if (RTN_Valid(memmoveRTN))
    {
        ...
        RTN_InsertCall(memmoveRTN, IPOINT_BEFORE, (AFUNPTR) GetArgMemmove ,IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,IARG_FUNCARG_ENTRYPOINT_VALUE, 2,
                       IARG_PTR, v,IARG_END);
    ...
    if (RTN_Valid(memcpyRTN))
    {
        ...
        // Instrument malloc() to print the input argument value and the return value.
        RTN_InsertCall(memcpyRTN, IPOINT_BEFORE, (AFUNPTR) GetArgMemcpy ,IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,IARG_FUNCARG_ENTRYPOINT_VALUE, 2,
                       IARG_PTR, v,IARG_END);
...
VOID record_diff(const CONTEXT *cpu, ADDRINT pc, VOID *v) {
    ...
    *OutFile << PC << "=0x" << pc;
    ...
    if (data->mem_w_size) {
        if (data->mem_mov_size && data->mem_mov_addr) {
            memset(data->m_scratch, 0, data->mem_mov_size);

            PIN_SafeCopy(data->m_scratch, (const VOID *) data->mem_mov_addr, data->mem_mov_size);
            *OutFile << ",mw=0x" << data->mem_mov_addr << ":";

            for (UINT32 i = 0; i < data->mem_mov_size; i++) {
                *OutFile << std::hex << std::setw(2) << std::setfill('0') << ((unsigned char) data->m_scratch[i] & 0xff);
            }
            data->mem_mov_addr = 0;
            data->mem_mov_size = 0;
